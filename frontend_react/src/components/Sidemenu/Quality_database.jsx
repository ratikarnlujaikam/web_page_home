...
import { Url_Qulity, Url_python } from "../../constants/index.jsx";
class Engineer extends Component {
  ...
  render() {
    ...
    const Url_1 = `${Url_Qulity}`;
    const Url_2 = `${Url_python}`;
    const Traceability =
      data && data.api_Quality
        ? data.api_Quality.filter((item) => item.Details === "Traceability")
        : [];

    const Monitoring =
      data && data.api_Quality
        ? data.api_Quality.filter((item) => item.Details === "Monitoring")
        : [];

    const Report =
      data && data.api_Quality
        ? data.api_Quality.filter((item) => item.Details === "Report")
        : [];

    const Analysis_requester =
      data && data.api_Quality
        ? data.api_Quality.filter(
      ...
      <div className="content-wrapper border border-blue-200 bg-white rounded-lg shadow-md p-6">
        ...
        <div className="w-full">
          <div className="card border border-blue-400 bg-blue-50 rounded-lg shadow-lg p-4">
            <h2 className="text-3xl font-bold text-blue-700 mb-6">Quality</h2>
